...
speech_endpoint = "https://swedencentral.api.cognitive.microsoft.com/"
...
def speech_synthesis_with_language():
    ...
    while True:
        ...
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            print(
                "Speech synthesized to speaker for text [{}] with language [{}]".format(
                    text, language
                )
            )
        elif result.reason == speechsdk.ResultReason.Canceled:
...
def speech_synthesis_with_voice():
    ...
    while True:
        ...
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            print(
                "Speech synthesized to speaker for text [{}] with voice [{}]".format(
                    text, voice
                )
            )
        elif result.reason == speechsdk.ResultReason.Canceled:
...
def speech_synthesis_to_push_audio_output_stream():
    ...
    print(
        "Totally {} bytes received.".format(stream_callback.get_audio_size())
    )
...
def speech_synthesis_to_audio_data_stream():
    ...
    while True:
        ...
        if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
            ...
            print(
                "Audio data for text [{}] was saved to [{}]".format(
                    text, file_name
                )
            )
...
def speech_synthesis_events():
    ...
    speech_synthesizer.synthesis_started.connect(
        lambda evt: print("Synthesis started: {}".format(evt))
    )
    speech_synthesizer.synthesizing.connect(
        lambda evt: print(
            "Synthesis ongoing, audio chunk received: {}".format(evt)
        )
    )
    speech_synthesizer.synthesis_completed.connect(
        lambda evt: print("Synthesis completed: {}".format(evt))
    )
...
def speech_synthesis_word_boundary_event():
    ...
    speech_synthesizer.synthesis_word_boundary.connect(
        lambda evt: print(
            "Word boundary event received: {}, audio offset in ms: {}ms".format(
                evt, evt.audio_offset / 10000
            )
        )
    )
...
def speech_synthesis_bookmark_event():
    ...
    speech_synthesizer.bookmark_reached.connect(
        lambda evt: print(
            "Bookmark reached: {}, audio offset: {}ms, bookmark text: {}.".format(
                evt, evt.audio_offset / 10000, evt.text
            )
        )
    )
